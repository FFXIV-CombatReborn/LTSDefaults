...
public sealed class MCH_zBeta : MachinistRotation
{
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? act)
    {
        ...
        bool OpenerQueen = IsLastGCD(true, ChainSawPvE);
        ...
        if (IsBurst)
        {
            ...
            // Wildfire Check
            if ((CombatElapsedLess(20f) && nextGCD.IsTheSameTo(true, ChainSawPvE)) || (IsLastAbility(false, HyperchargePvE) || Heat >= 50) && /*!CombatElapsedLess(10) &&*/ CanUseHyperchargePvE(out _)
            && !LowLevelHyperCheck)
            {
                if (WildfirePvE.CanUse(out act, onLastAbility: true)) return true;
            }
        }
